...
const AwaitedCandidates = () => {
    ...
    const fetchAwaitApplication = async (page = 1) => {
      if (!jobId) {
      ...
      try {
        const response = await jobpostAxios.get(`${jobpostEndpoints.viewAwaitApplication}?jobId=${jobId}&page=${page}&limit=2`);
    ...
    useEffect(() => {
        fetchAwaitApplication(currentPage);
...
export default AwaitedCandidates